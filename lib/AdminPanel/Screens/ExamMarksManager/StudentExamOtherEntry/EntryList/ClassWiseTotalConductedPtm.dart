...
import 'package:digivity_admin_app/AdminPanel/Screens/ExamMarksManager/StudentExamOtherEntry/StudentCard/ClasswiseConductedptmCard.dart';
...
class ClassWiseTotalConductedPtm extends StatefulWidget {
  final String? courseId;
  final int examTermId;
  final String? course;
  final String? examTerm;
  ClassWiseTotalConductedPtm({
  ...
  @override
  State<ClassWiseTotalConductedPtm> createState() {
    return _ClassWiseTotalAttendance();
  }
}

class _ClassWiseTotalAttendance extends State<ClassWiseTotalConductedPtm> {
  Map<String, TextEditingController> conductedptmControllers = {};
  String? userId;
  List<ClasswiseTotalConductedPtmModel> classList = [];
  ...
  Future<void> getStudentList() async {
    ...
    final response = await StudentExamOtherEntryHelper()
        .apiclasswisetotalconductedptm(bodydata);

    if (response != null) {
      setState(() {
        classList = response;
        for (var cls in classList) {
          conductedptmControllers[cls.key.toString()] = TextEditingController(
            text: cls.classTotalPtm.toString(),
          );
        }
      });
    }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: PreferredSize(
        ...
        child: SimpleAppBar(
          titleText: 'Class Wise Total Conducted PTM',
          routeName: 'back',
        ),
      ),
            ...
            Expanded(
              child: classList.isEmpty
                ...
                itemBuilder: (context, index) {
                  ...
                  final controller = conductedptmControllers[classItem.key.toString()]!;
                  return ClasswiseConductedptmCard(
                    courseId: classItem.key.toString(),
                    courseName: classItem.value.toString(),
                    conductedptmController: controller,
                  );
                },
              ),
            )
        ...
        child: CustomBlueButton(
            ...
            onPressed: () async {
              ...
              for (var classItem in classList) {
                ...
                final totalconductedptm = conductedptmControllers[classId]?.text ?? '0';
                final userId = (await SharedPrefHelper.getPreferenceValue('user_id'))?.toString() ?? "";

                updatedatalist.add({
                  "user_id":userId,
                  "course_section_id": classId,
                  "total_conducted_ptm": totalconductedptm,
                });
              }
              ...
              final response = await StudentExamOtherEntryHelper().storeStudentClasswiseCondcutedPTM(bodydata);
